 ...
 */
/**
 * Lectures d'objets menées de front.
 *
 * Chaque `io.read` d'un conteneur `v2:` coûte une dérivation PBKDF2-SHA512 à
 * 600 000 tours — mesuré 265 ms sur un i5-13600K. Lues EN SÉRIE, 32 notes
 * faisaient 8,5 s au démarrage (journal du 05/09/2026 : 8,3 s entre
 * « Initialized for profile » et « v2 — 32 note(s) »), et le coût croît avec le
 * nombre de notes : 500 notes, plus de deux minutes. La dérivation tourne sur le
 * pool libuv, et celui-ci a QUATRE fils : Electron le dimensionne avant que
 * notre code tourne, donc `UV_THREADPOOL_SIZE` posé en tête de main.ts n'a rien
 * changé (mesuré : 3,0 s pour 32 notes, soit exactement quatre de front). Huit
 * garde la file du pool pleine sans empiler inutilement ; on ne fera pas mieux
 * que ×3 sans changer le format.
 *
 * Le vrai remède est ailleurs : un conteneur `v3:` dérivé par HKDF (la clé
 * machine est déjà aléatoire, PBKDF2 ne protège rien ici) — mais c'est un
 * format lu aussi par le web et le mobile, donc un contrat à geler d'abord.
 */
export const VAULT_READ_CONCURRENCY = 8;

/**
 * Applique `fn` à chaque élément avec au plus `limit` appels en vol. Le premier
 * échec rejette — comme la boucle `for … await` qu'elle remplace.
 */
...
export async function loadVaultV2(io: VaultIO): Promise<LoadedVault | null> {
   ...
   * déchiffre en n'importe quel objet (déchiffrement à moitié réussi, fichier
   * d'une autre nature posé là) devenait un index VIDE et VALIDE : le coffre
  ...
  if (!isNotesIndexShape(raw)) {
    throw new Error(
      "notesVaultStore: index.enc n'a pas la forme d'un index v2 — lecture refusée"
    );
  }
  const index = normalizeIndex(raw);

  // Lues de front, puis rangées dans l'ORDRE DE L'INDEX : l'ordre d'arrivée
  // varie d'un chargement à l'autre, et une empreinte du clair calculée sur
  // `byId` ne doit pas bouger quand rien n'a changé.
  const lisibles = Object.entries(index.notes).filter(([, e]) => isValidObjectId(e.objectId));
  const lus = new Map<string, NoteRecord>();
  await readBounded(lisibles, VAULT_READ_CONCURRENCY, async ([noteId, entry]) => {
    const note = await io.read(notePath(entry.objectId));
    if (note && typeof note === 'object' && !Array.isArray(note)) {
      lus.set(noteId, note as NoteRecord);
    }
  });
  const notes: Record<string, NoteRecord> = {};
  for (const [noteId] of lisibles) {
    const note = lus.get(noteId);
    if (note) notes[noteId] = note;
  }
...
export async function writeNoteObject(
  ...
  if (!isValidObjectId(objectId)) {
    throw new Error(`notesVaultStore: identifiant d'objet refuse : ${String(objectId).slice(0, 32)}`);
  }
 ...
 *
 * ⚠ RIEN DE CE QUI EST IMBRIQUÉ N'EST CANDIDAT. Les images vivent sous
 * `notes/blobs/`, et l'index des notes ne les réclame pas — elles seraient donc
 * toutes vues comme orphelines. Aujourd'hui `fs.readdir` ne descend pas, donc
 * elles ne remontent pas dans la liste : la sûreté ne tient qu'à ça, et rien
 * ne l'écrivait. Une seule ligne suffit à la rendre explicite plutôt que
 * dépendante d'un détail de `readdir`.
 *
 * Le ménage des images a son propre chemin, avec ses propres refus
 * (`selectSweepableBlobs`) — c'est là que ça se décide, pas ici.
 */
