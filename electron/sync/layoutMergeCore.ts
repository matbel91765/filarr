/**
 * Fusion du CONTENEUR DE MISE EN PAGE — module PUR (ni disque, ni crypto, ni
 * Electron), utilisable tel quel par les tests.
 *
 * ⚠ MODÈLE DE DONNÉES FIGÉ ICI, et porté à l'identique côté renderer dans
 * `src/services/layout/layoutTypes.ts`. La duplication n'est pas un oubli :
 * le processus principal se compile avec `electron/tsconfig.json`, dont la
 ...
 * casse `package.json#main`. Exactement la raison d'être de `notesMergeCore.ts`.
 *
 * ── DEUX NOTIONS SÉPARÉES, et c'est tout le sujet ───────────────────────────
 *
 *  · L'INSTANCE (`LayoutView`) : ce que CET utilisateur a posé sur CETTE vue
 *    (`home`, `folder:<uuid>`, `home:alt`). Elle est liée à des identifiants
 *    LOCAUX (dossiers, notes) par `binding`, elle se synchronise entre ses
 *    appareils, et elle n'est JAMAIS exportée telle quelle.
 *
 *  · Le GABARIT (`LayoutTemplate`) : une disposition PARTAGEABLE dont chaque
 *    emplacement porte un RÔLE SYMBOLIQUE (`recents`, `favorites`,
 *    `folder-grid`, `stats`…) et jamais un UUID. C'est lui, et lui seul, qui
 *    voyage d'un utilisateur à l'autre. `instantiate` (côté renderer, voir
 *    `layoutTypes.ts`) transforme un gabarit + un contexte en instance.
 *
 * ── GRAIN DE FUSION : L'ENTRÉE DE VUE ───────────────────────────────────────
 *
 * Une vue est un objet ENTIER. Elle n'est JAMAIS fusionnée widget par widget :
 * deux appareils qui déplacent chacun un bloc produiraient, par union
 * positionnelle, une disposition avec des chevauchements que PERSONNE n'a
 * voulue — un résultat que ni l'un ni l'autre ne reconnaîtrait. On tranche donc
 * au grain de la vue, comme `SyncProfileMeta` tranche au grain du profil.
 *
 * ARBITRAGE : LWW STRICT sur `updatedAt`, calqué sur le patron déjà en place
 * pour `applyCloudMetaUpdate` :
 *   - le distant n'est appliqué que s'il est STRICTEMENT plus frais ;
 *   - l'ÉGALITÉ conserve le local ;
 *   - une horloge ABSENTE ou illisible n'est PAS autoritaire (elle vaut -∞ et
 *     perd face à toute horloge lisible ; deux -∞ = le local reste).
 *
 * CE QUE L'ARBITRAGE DÉTRUIRAIT SANS FILET : la disposition perdante. Elle est
 * donc CONSERVÉE dans `superseded` (30 jours, 5 au plus par vue) — l'interface
 * proposera plus tard « Garder celui-ci / Prendre l'autre ». Rien n'est perdu
 * en silence, et la liste s'oublie d'elle-même.
 */
...
function toSuperseded(view: LayoutView, side: 'local' | 'remote', nowIso: string): SupersededLayout {
...
export function mergeLayoutDocuments(
  ...
  if (docLocal !== null || docRemote !== null) {
    merged.updatedAt = (docRemote ?? -Infinity) > (docLocal ?? -Infinity)
      ? remote.updatedAt
      : local.updatedAt;
  }

  /*
    `updatedAt` NE DÉCIDE PAS D'UNE REMONTÉE — et l'oublier a fait boucler deux
    postes toute une nuit.

    Ce champ est INFORMATIF, la ligne au-dessus le dit. Mais il participait à
    l'égalité qui décide s'il faut republier : la fusion garde le plus récent des
    deux horodatages, donc l'appareil dont le conteneur est le plus récent trouve
    toujours `merged ≠ remote` et pousse. L'autre reçoit, refait la même
    constatation dans l'autre sens, et pousse à son tour.

    Contenu IDENTIQUE des deux côtés, et republication sans fin. Mesuré le
    2026-09-08 : la version du manifeste montait de 24 par minute
    (12115 → 12298 en sept minutes), ce qui faisait perdre le compare-and-set à
    tous les cycles et transformait chaque désaccord d'empreinte en conflit.

    C'est la même famille que le `plaintextChecksum` du fusionneur de fichiers :
    comparer ce qui PORTE DU SENS, jamais ce qui n'est qu'une estampille.
  */
  const sansEstampille = (d: LayoutDocument): LayoutDocument => {
    const { updatedAt: _ignore, ...reste } = d;
    return reste as LayoutDocument;
  };

  return {
    merged,
    // `changedFromLocal` garde l'estampille : elle décide d'une ÉCRITURE
    // locale, et réécrire un fichier identique ne coûte qu'un accès disque.
    // C'est la remontée qui se paie en réseau et en réveils de l'autre poste.
    changedFromLocal: !deepEqual(merged, local),
    changedFromRemote: !deepEqual(sansEstampille(merged), sansEstampille(remote)),
    conflicts,
  };
}
