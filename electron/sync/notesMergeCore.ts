...
/**
 * Fusion NOTE À NOTE du store de notes — module PUR (ni disque, ni crypto, ni
 * Electron), utilisable tel quel par les tests.
 *
 * ⚠ SOURCE DE VÉRITÉ : `src/platform/web/sync/notesMerge.ts`.
 * Ce fichier en est un PORTAGE À L'IDENTIQUE, pas une variante. Il existe parce
 * que le processus principal se compile avec `electron/tsconfig.json`, dont la
 * racine est `electron/` : importer un module de `src/` déplacerait la racine
 * commune du programme et donc TOUTE l'arborescence émise dans `dist-electron`
 * (`dist-electron/electron/main.js` au lieu de `dist-electron/main.js`), ce qui
 * casse `package.json#main`. Toute évolution de la règle de fusion doit être
 * portée ici, et `electron/sync/__tests__/notesMergeCore.vitest.ts` compare les
 * deux implémentations sur un jeu de cas partagé pour que la dérive se voie.
 *
 ...
 * les registres ne grossissent pas sans fin.
 *
 * INVARIANT DUR : `byId` fusionné ne peut pas contenir MOINS d'entrées que le
 * local, purges mises à part — une tombstone remplace la note mais garde sa clé.
 * `mergeNotesPayload` le vérifie et jette plutôt que de rendre un store amputé ;
 * l'appelant (syncService) n'écrit alors RIEN et garde le local intact.
 *
 ...
 * que `handleConflict` fabrique déjà pour les fichiers ordinaires.
 */
...
export interface NotesMergeResult {
  merged: NotesPayload;
  /** La fusion diffère du store LOCAL → il faut réécrire `notes.enc`. */
   ...
   * purgée). SEUL ce drapeau justifie de prévenir le renderer : `notes-updated`
...
/** Clé de l'entrée des notes dans le manifeste de synchronisation. */
...
/** Nom du blob de notes sur le disque, à la racine du dossier de profil. */
export const NOTES_BLOB_FILENAME = 'notes.enc';
...
/** Durée de vie d'une pierre tombale de purge (miroir du web). */
export const PURGE_TOMBSTONE_TTL_MS = 90 * 24 * 60 * 60 * 1000;

/**
 * Version du protocole de fusion des notes annoncée par ce desktop dans le
 * manifeste (`SyncManifest.notesMergeVersion`). C'est un MARQUEUR DE CAPACITÉ :
 * il dit « le dernier appareil qui a écrit ce manifeste fusionne les notes note
 * à note au lieu d'écraser le blob ». Un desktop antérieur à cette version ne
 * connaît pas le champ, ne le recopie donc pas dans le manifeste qu'il pousse :
 * son absence est le signal de repli sûr.
 */
export const NOTES_MERGE_VERSION = 1;
 ...
 *
 * `updatedAt` seul ne suffit pas : la suppression douce pose `deletedAt` SANS
 * toucher `updatedAt`. Une tombstone aurait donc exactement l'horloge de la note
 * vivante d'en face, l'égalité garderait le local, et une suppression faite sur
 * le web ressusciterait sur le desktop — puis serait repoussée vers le web.
 */
 ...
 * face à une note locale VIVANTE, une tombstone distante doit être STRICTEMENT
 * plus fraîche pour la re-supprimer ; à horloge égale, la restauration tient.
 */
 ...
 * conservée. Rend les ids réellement retirés.
 */
 ...
 * conservées, dans leur ordre (doublons d'id compris) ; une entrée distante de
 * même id n'écrase la première occurrence locale que si son horloge est
 * strictement plus fraîche ; une entrée distante sans id n'est ajoutée que si
 * aucune entrée sans id ne lui est identique.
 */
...
export function mergeNotesPayload(
  ...
  let purgedFromLocal = 0;
  if (byId) {
    const removed = applyPurges(byId, purgedNotes);
    if (isPlainObject(local.byId)) {
      for (const id of removed) if (has(local.byId, id)) purgedFromLocal++;
    }
    merged.byId = byId;
  }
  ...
  // 7. Filet : une fusion ne peut pas rendre moins de notes que le local — à
  //    l'exception EXACTE des notes qu'une purge définitive a retirées.
  if (
    isPlainObject(local.byId) &&
    Object.keys(byId ?? {}).length + purgedFromLocal < Object.keys(local.byId).length
  ) {
    throw new Error('Fusion des notes refusée : le résultat contient moins de notes que le local');
  }
 ...
 *
 * ⚠ PORTAGE À L'IDENTIQUE de `src/platform/web/sync/notesMerge.ts`.
 *
 ...
 *
 * La liste des notes vivantes vient du renderer (`collab:setLiveNotes`) : c'est
 * lui qui tient les sessions, le processus principal ne fait que l'appliquer.
 */
 ...
 * quand même — une par cycle, chacune portant le texte tapé à cet instant.
 *
 ...
 * l'arbitrage est de toute façon REPORTÉ (voir `preserveLiveSessionNotes`).
 *
 ...
 * fausse dès qu'un cycle fusionnait SANS remonter (remontée différée, note
 * retenue par une session vivante, envoi échoué, appareil hors ligne) :
 * l'ancêtre avançait alors sur nos écritures locales que le nuage n'avait jamais
 * vues, et au cycle suivant l'horloge distante — restée en arrière, donc
 * « différente de l'ancêtre » — passait pour un mouvement. Le local ayant
 * vraiment bougé, une copie de conflit naissait d'une note que personne d'autre
 * n'avait touchée.
 *
...
export function collectMergeBase(localPayload: unknown, remotePayload: unknown): NotesMergeBase {
  ...
  return { notes: ours.notes, notebooks: ours.notebooks, remote: collectEntryClocks(remotePayload) };
}
 ...
 *
 * POURQUOI UNE NOTE ET PAS UN INSTANTANÉ `noteVersionService`. Son API publique
 * ne sait pas archiver une version ARBITRAIRE : `recordSnapshots` part des notes
 * telles que le renderer les tient, déduplique par empreinte et s'auto-limite
 * dans le temps — une version distante perdue y serait silencieusement écartée.
 * La copie de conflit prend donc la même forme que sur le web : une note, avec
 * un id NEUF (réutiliser l'id d'origine écraserait la version gagnante), qui se
 * lit, se compare, se synchronise et se supprime comme n'importe quelle note.
 * Cet id n'existant que d'un seul côté, l'union le recopie sans jamais
 * l'arbitrer — la copie ne peut pas engendrer un second conflit au cycle suivant.
 */
export function applyConflictCopies(
  payload: NotesPayload,
  entries: OverwrittenEntry[],
  options: { now: string; newId: () => string; titleSuffix?: string; device?: string }
): number {
