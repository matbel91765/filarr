/**
 * Pure logic for note version history — MAIN-PROCESS COPY.
 *
 * IMPORTANT: this file is a byte-for-byte duplicate of
 * `src/services/notes/noteVersionLogic.ts`. The renderer and main
 * process can't share a single source file without extending
 * electron's `rootDir` (which breaks the `dist-electron/main.js`
 * layout expected by `package.json "main"`). The file is small and
 * stable, so we duplicate and rely on the tests in `src/services/
 * notes/__tests__/` to pin the invariants.
 *
 * If you edit one, edit the other. Structurally identical.
 *
...
export function effectiveRetentionDays(
  ...
  return typeof orgDays === 'number' && Number.isFinite(orgDays) && orgDays > 0 ? orgDays : defaultDays;
}
