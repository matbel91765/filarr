/**
 * Connecteurs de bases inline — LISTE BLANCHE des sources amont.
 *
 * ⚠ SOURCE DE VÉRITÉ : `src/platform/connectors/connectorSources.ts`.
 * Ce fichier en est un PORTAGE À L'IDENTIQUE (en-tête excepté), pas une
 * variante. Il existe parce que le processus principal se compile avec
 * `electron/tsconfig.json`, dont la racine est `electron/` : importer un
 * module de `src/` déplacerait la racine commune du programme et donc toute
 * l'arborescence émise dans `dist-electron` (même contrainte que
 * `electron/sync/notesMergeCore.ts`).
 * Toute évolution de la liste blanche doit être portée ici, et le test de
 * parité
 * `electron/connectors/__tests__/connectorSourcesCore.vitest.ts`
 * compare les deux implémentations sur un jeu d'entrées partagé pour que la
 * dérive se voie.
 *
 * Rappel du principe : le client n'envoie JAMAIS d'URL — il envoie
 * `{ source, query, apiKey?, lang? }` et l'URL amont est fabriquée ici, à
 * partir d'une table figée. C'est la garde anti-SSRF.
 */
...
export function isConnectorSourceId(value: unknown): value is ConnectorSourceId {
  return (
    typeof value === 'string' && (CONNECTOR_SOURCE_IDS as readonly string[]).includes(value)
  );
}
