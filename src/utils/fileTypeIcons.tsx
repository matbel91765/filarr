...
/**
 * Un DOCUMENT — page écrite, distincte de la page vierge générique.
 *
 * `.fdoc`, le format natif de Filarr, tombait jusqu'ici sur l'icône générique :
 * le seul format que ce produit possède était le seul à n'avoir aucun signe
 * distinctif dans son propre explorateur.
 */
const DocumentFileIcon: FC<{ size?: number }> = ({ size = 48 }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    fill="none"
    viewBox="0 0 24 24"
...
const DOC_EXT = new Set(['fdoc', 'docx', 'doc', 'odt', 'rtf']);
...
export const getFileTypeInfo = (
  ...
  // AVANT le test « code » : `mimeType?.startsWith('text/')` y attraperait un
  // `.rtf` (text/rtf) et lui donnerait l'icône d'un fichier source.
  if (DOC_EXT.has(ext)) {
    return { icon: <DocumentFileIcon size={size} />, color: TYPE_COLORS.document };
  }
