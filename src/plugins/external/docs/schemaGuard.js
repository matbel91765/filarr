 ...
 */
export function peersSupportSchema(states, selfClientId, requis = DOCS_SCHEMA) {
    for (const [clientId, state] of states) {
        if (clientId === selfClientId)
            continue;
        const annonce = state?.[SCHEMA_FIELD];
        // Un pair d'une version antérieure n'annonce RIEN : l'absence est la
        // réponse, et c'est « non ». Un NaN non plus n'est pas une annonce —
        // `NaN < requis` est faux, ce qui aurait dit « oui » par accident.
        if (typeof annonce !== 'number' || !Number.isFinite(annonce) || annonce < requis)
            return false;
    }
    return true;
}
 ...
 */
export function announceSchemaCapability(awareness, onChange) {
    awareness.setLocalStateField(SCHEMA_FIELD, DOCS_SCHEMA);
    let ready = peersSupportSchema(awareness.getStates(), awareness.clientID);
    const handler = () => {
        const suivant = peersSupportSchema(awareness.getStates(), awareness.clientID);
        if (suivant === ready)
            return;
        ready = suivant;
        onChange(ready);
    ...
    return {
        ...
        destroy: () => {
            try {
                ...
                /* awareness déjà détruite avec la salle */
            }
        },
    };
}
 ...
 * la salle sûre.
 */
export function claimSchemaFloor(doc) {
    const map = doc.getMap(SCHEMA_MAP);
    const brut = map.get(SCHEMA_KEY);
    const actuel = typeof brut === 'number' && Number.isFinite(brut) ? brut : 0;
    if (actuel > DOCS_SCHEMA) {
        // Refuser, jamais dégrader : se lier ici détruirait ce que v4 sait porter.
        return { mode: 'solo', roomSchema: actuel };
    }
    if (actuel < DOCS_SCHEMA)
        map.set(SCHEMA_KEY, DOCS_SCHEMA);
    return { mode: 'join', roomSchema: actuel };
}
