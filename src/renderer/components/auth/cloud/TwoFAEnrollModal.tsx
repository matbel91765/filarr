/**
 * TwoFAEnrollModal — E5-7 forced 2FA enrolment.
 *
 * Shown at LOGIN when the server reports `mfaEnrollmentRequired` (the user's org mandates 2FA and they
 * have none). Same three phases as the voluntary TwoFASetupModal — qr → verify → codes — but it runs on
 * the restricted enrol token held in the Electron main process (no session exists yet), and finishing
 * does NOT log the user in: enroll-verify mints no session by design, so `onEnrolled` triggers a normal
 * re-login (which now hits the standard 2FA challenge). Cancelling aborts the login.
 */
...
interface TwoFAEnrollModalProps {
  isOpen: boolean;
  onCancel: () => void;
  onEnrolled: () => void;
}
...
const TwoFAEnrollModal: React.FC<TwoFAEnrollModalProps> = ({ isOpen, onCancel, onEnrolled }) => {
  ...
  // The 5-min enrol token expired / was dropped mid-flow → the only way forward is a fresh login.
  const [expired, setExpired] = useState<boolean>(false);

  // "log in again" is the wording both the server and main process use when the enrol token is gone.
  const looksExpired = (msg?: string | null) => !!msg && /log in again|expired|expiré/i.test(msg);
  ...
  const runSetup = useCallback(async () => {
    ...
    setQrDataUrl('');
    setBusy(true);
    const result = await authApi.mfaEnrollSetup();
    if (!result.success || !result.otpauthUrl) {
      if (looksExpired(result.error)) {
        setExpired(true);
      } else {
        setError(result.error || t('settings.2fa.enrollSetupFailed', 'Could not start 2FA setup'));
      }
      setBusy(false);
      return;
    }
  ...
  const handleVerify = async () => {
    ...
    setBusy(true);
    setError(null);
    const result = await authApi.mfaEnrollVerify(code);
    setBusy(false);
    if (!result.success || !result.backupCodes) {
      // A gone/expired token can't be retried in place — route the user back to login.
      if (looksExpired(result.error)) setExpired(true);
      else setError(result.error || t('settings.2fa.invalidCode', 'Invalid code'));
      return;
    }
  ...
  return (
    <Modal
      ...
      title={t('settings.2fa.enrollTitle', 'Votre organisation exige la double authentification')}
      size="md"
    >
      <ModalBody>
        {expired && (
          <div>
            ...
            >
              ...
              <p>
                {t(
                  'settings.2fa.enrollExpired',
                  "Votre session d'inscription a expiré. Reconnectez-vous pour réessayer."
                )}
              </p>
            </div>
            <button
            ...
            >
              {t('settings.2fa.enrollBackToLogin', 'Retour à la connexion')}
            </button>
          </div>
        )}
        {!expired && phase === 'qr' && (
          <div>
            ...
            >
              ...
              <p>
                {t(
                  'settings.2fa.enrollIntro',
                  'Un administrateur de votre organisation impose la double authentification (2FA). Configurez-la maintenant pour terminer votre connexion.'
                )}
              </p>
            </div>
            ...
            {error && (
              <div className="p-3 mb-3 rounded-lg bg-red-50 border-l-4 border-red-500 text-sm text-red-700">
                {error}
              </div>
            )}
            <div className="flex gap-2">
              ...
              {error && !qrDataUrl ? (
                ...
                <button
                  ...
                  style={{
                    backgroundColor: 'var(--color-primary-600)',
                    cursor: busy || !qrDataUrl ? 'not-allowed' : 'pointer',
                    opacity: busy || !qrDataUrl ? 0.5 : 1,
                  }}
                >
        ...
        {phase === 'codes' && (
          <div>
            ...
            >
              {t('settings.2fa.enrollDoneLogin', 'Terminer et se connecter')}
            </button>
          </div>
        )}
      </ModalBody>
