...
export const TemplateManager: React.FC<TemplateManagerProps> = React.memo(
  function TemplateManager({ isOpen, onClose }) {
    ...
    const handleUseTemplate = useCallback(
      (template: NoteTemplate) => {
        dispatch(createNewNote({ templateId: template.id, title: template.name }));
        onClose();
      },
      [dispatch, onClose]
    );
      ...
      try {
        ...
        const extractText = (nodes: any[]) => {
          for (const node of nodes) {
            ...
            if (node.content && !['heading', 'paragraph', 'taskList', 'bulletList'].includes(node.type)) {
      ...
      <div className="template-manager__overlay" onClick={onClose}>
        <div className="template-manager" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="template-manager__header">
            ...
            <button className="template-manager__close" onClick={onClose}><CloseIcon /></button>
          </div>
                  ...
                  {builtIn.map((tpl) => (
                    ...
                    >
                      <span className="template-manager__item-icon">
                        {TEMPLATE_ICONS[tpl.icon] || TEMPLATE_ICONS.default}
                      </span>
                      <div className="template-manager__item-info">
                        <span className="template-manager__item-name">{tpl.name}</span>
                        <span className="template-manager__item-desc">{tpl.description}</span>
                      </div>
                    </button>
                  ))}
                </>
            ...
            <div className="template-manager__preview">
              {selectedTemplate ? (
                <>
                  ...
                  <pre className="template-manager__preview-content">{previewContent || '(empty template)'}</pre>
