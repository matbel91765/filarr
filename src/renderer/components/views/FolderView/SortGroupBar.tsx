...
// Tri / liste / grille : les MÊMES glyphes que la vue de coffre partagé, tirés
// de `files/explorerIcons` et plus recopiés (lot A, C6). Mêmes 16 px qu'avant.
import { SortAscIcon, SortDescIcon, ListIcon, GridIcon } from '../../files/explorerIcons';
...
export const SortGroupBar: React.FC<SortGroupBarProps> = ({
  searchQuery,
  onSearchChange,
  className,
}) => {
  ...
  const GROUP_OPTIONS: Array<{ value: 'none' | 'type' | 'date' | 'firstLetter'; label: string }> =
    useMemo(
      () => [
  ...
  const currentSortLabel =
    SORT_OPTIONS.find((o) => o.value === sortBy.field)?.label || t('sort.name');
  const currentGroupLabel =
    GROUP_OPTIONS.find((o) => o.value === groupBy.field)?.label || t('sort.none');
  ...
  return (
    <div
      className={`flex items-center justify-between px-6 py-2.5 bg-[var(--color-surface)] border-b border-[var(--color-border-light)] shrink-0 ${className || ''}`}
    >
      {/* Search */}
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={() =>
            dispatch(
              setSortBy({ field: sortBy.field, order: sortBy.order === 'asc' ? 'desc' : 'asc' })
            )
          }
